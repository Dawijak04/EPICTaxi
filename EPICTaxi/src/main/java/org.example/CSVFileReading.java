...
public class CSVFileReading {
    static String csvPath = "src/TaxiFile.txt";
    ...
    public static void updateRating() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(csvPath))) {
            ...
            double newRating = ((UserInterface.getAssignedTaxi().getRating() * UserInterface.getAssignedTaxi().getNoOfTrips()) + //drivers current rating mulitplied by number of trips
                    Double.parseDouble((Double.toString(Person.getRating())))) / //plus drivers new rating
                    (UserInterface.getAssignedTaxi().getNoOfTrips() + 1); //divided by drivers number of trips + 1
            ...
            while ((line = br.readLine()) != null) { //while file isnt finished
                ...
                if (data.length >= 7 && data[1].equals(UserInterface.getAssignedTaxi().getReg())) { //if appropriate format and has reg of assigned taxi
                    data[5] = Double.toString(newRating); //set sixth value as the new rating
                    data[6] = String.valueOf(UserInterface.getAssignedTaxi().getNoOfTrips() + 1); //set seventh value as new number of trips
                }

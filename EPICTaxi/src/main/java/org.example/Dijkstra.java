...
public class Dijkstra {
    ...
    public static DataList<Node> buildPath(Node start, Node end, DataHashMap<Node, Node> predecessorMap) {
    ...
    public static boolean isRiver(Node node) { //checks if node is a river
        int x = node.getData() % 10; //gets x coordinate
        int y = node.getData() / 10; //gets y coordinate

        return LinkedGrid.isRiver(x, y); //runs isRiver method from LinkedGrid
    }
    ...
    public static Node getNodeAt(int x, int y) { //gets node at a given coordinate
